...
1. 如果页面提示初始化未完成，点击 `一键初始化`。
2. 等待任务完成。这里会下载 localClash 核心、Mihomo 和 Dashboard。
3. 如果页面提示等待订阅，点击 `填写订阅`。
4. 一行填写一个订阅 URL。
5. 点击 `保存并应用订阅`。
6. 回到概览页，确认核心、Mihomo、Dashboard、订阅都不是缺失状态。
7. 确认要接管路由器流量后，点击 `启动运行时并接管路由器流量`。

最后一步会影响整台路由器的网络。如果这台路由器正在用 OpenClash、PassWall 或其他插件维持上网，先不要急着接管，先确认自己知道怎么恢复原来的插件。
